...
public class Imple implements Inter {
	...
	public int responsinter(Uploadbean rp) {
		...
		public int tlf2(tlfilebean2 tf) {
			...
			{
				 con=Dbconn.create();
				 PreparedStatement ps=con.prepareStatement("INSERT INTO ipspoof.stafffileadd VALUES(id,?,?,?,?,?,?,?,?,?,?,?,?,?)");
				 ...
				 ps.setString(9, tf.getS1mail());
			
				
				 ps.setString(10, "empty");
				 ps.setString(11, tf.getPhash());
				 ps.setString(12, tf.getAhash());
				 ps.setString(13, "ok");
				 dd=ps.executeUpdate();	
			}
